<script lang="ts">
	...
	import type { MainMenu } from '$lib/modules/mini-games/game/scenes/MainMenu';
	import PhaserGame, { type TPhaserRef } from '../PhaserGame.svelte';

	// TODO : Add the function to grow the size off the scrren if it's too small
	// TODO : Add a way to reset the game (restart the phaser game instance) ?

	let numberOfSpritesAdded = 0;
	...
	const resetInfo = () => {
		// reset the sprite position info
		spritePosition.x = 0;
		spritePosition.y = 0;
		// reset the sprite added counter
		numberOfSpritesAdded = 0;
	};
	...
	const addSprite = () => {
		...
		if (scene) {
			...
			const spriteChoosen = Phaser.Math.RND.pick(['star', 'stars']);
			//  `add.sprite` is a Phaser GameObjectFactory method and it returns a Sprite Game Object instance
			const spriteToAdd = scene.add.sprite(x, y, spriteChoosen);
...
<main class="container h-full max-h-full min-h-0 flex flex-col items-center justify-center mx-auto p-4 gap-2">
	<h1>Welcome to <span class="text-orange-400 font-semibold">Mini Games</span></h1>
	<div id="app" class="border-2 border-black-border rounded-md bg-grey-light">
